...
*** vendor_payment_amount - decimal(8,2)
    ...
    (even though it doesn't actually go there, for some reason, to be
    discovered later, probably having something to do with the blank
    "merchmailtemplate" field or "confemailtemplate" field)
**** VENDORQA VendorInvoice table
...
*** Vendor Change History
    ...
    - VendorEditHistory - Some of the edits
...
*** Returning Vendor informational update
    ...
    is set for this event as set.
     - SuperVendor cam also update anyone here.
...
**** VENDORQA in pieces
     ...
     - second report - just those with possible PDFs waiting to be
       checked and promoted.
