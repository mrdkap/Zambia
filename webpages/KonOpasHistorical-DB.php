...
if ($conrows > 1) {
  $message.="<P>Something wrong, more than one row with conid=$conid was returned</P>\n<P>";
  $message.=$query;
  $message.="</P>\n";
}

for ($i=1; $i<=$conrows; $i++) {
  $conname_array=mysqli_fetch_assoc($result);
  $conname=$conname_array['conname'];
  $connamelong=$conname_array['connamelong'];
  $connumdays=$conname_array['connumdays'];
  $Grid_Spacer=$conname_array['congridspacer'];
  $ConStart=$conname_array['constartdate'];
  $conurl=$conname_array['conurl'];
  $logo=$conname_array['conlogo'];
}
...
<h1><?php echo "<A HREF=\"http://$conurl\" target=\"_blank\" alt=\"$connamelong\" title=\"$connamelong\">" ?>
...
<p>This is the program guide for <?php echo "<A HREF=\"http://$conurl\" target=\"_blank\">$connamelong</A>" ?>. It should be suitable for use on most browsers and devices. It is an instance of <a href="http://konopas.org/" target="_blank">KonOpas</a>, an open-source project providing conventions with easy-to-use mobile-friendly guides.
