...
EOD;
  ...
  if ($vannualrows == 0) {
    ...
    if ($work_arr['wasvendornotes'] != $work_arr['vendornotes']) {
      $set_array[]="vendornotes='".mysqli_real_escape_string($link,stripslashes(htmlspecialchars_decode($work_arr['vendornotes'])))."'";
    }
    if (!empty($set_array)) {
...
function edit_vendor_update ($work_arr) {
  ...
  if ($badgeid == "100") {
    ...
    if ($work_arr['wasbadgename'] != $work_arr['badgename']) {
      $pairedvalue_array[]=("pubsname='".stripslashes(mysqli_real_escape_string($link,stripslashes($work_arr['badgename']."'"))));
    }

    if ($work_arr['wasemail'] != $work_arr['email']) {
      $pairedvalue_array[]=("email='".stripslashes(mysqli_real_escape_string($link,stripslashes($work_arr['email']."'"))));
    }
	...
	$pairedvalue_array[]=($element_array[$i]."='".stripslashes(mysqli_real_escape_string($link,stripslashes($work_arr[$element_array[$i]]."'"))));
      }
    }
