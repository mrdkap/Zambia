...
SELECT 
    ...
    numkidsfasttrack
  FROM
      $ReportDB.ParticipantAvailability
  WHERE
    conid=$conid AND
    badgeid=$badgeid
EOD;
  ...
  if (CON_NUM_DAYS>1) {
    $query="SELECT badgeid, day, maxprog FROM $ReportDB.ParticipantAvailabilityDays WHERE conid=\"$conid\" AND badgeid=\"$badgeid\"";
    $result=mysql_query($query,$link);
    if (!$result) {
      $message_error=$query."<BR>\n".mysql_error($link);
      return (-3);
  ...
  $query="SELECT badgeid, availabilitynum, starttime, endtime FROM $ReportDB.ParticipantAvailabilityTimes ";
...
SELECT 
    ...
    elementid
  FROM
      $ReportDB.PermissionAtoms PA,
      $ReportDB.Phase PH,
      $ReportDB.PermissionRoles PR,
